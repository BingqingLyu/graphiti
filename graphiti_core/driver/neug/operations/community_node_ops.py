...
from graphiti_core.driver.neug.parsers import community_node_from_record
...
class NeuGCommunityNodeOperations(CommunityNodeOperations):
    async def save(
        self,
        ...
        query = get_community_node_save_query(GraphProvider.NEUG)
    ...
    async def delete_by_uuids(
        ...
        query = (
            ...
            MATCH (c:Community)
            WHERE """
            + neug_in_filter('c.uuid', uuids, params)
            + """
            DETACH DELETE c
            """
        )
    ...
    async def get_by_group_ids(
        ...
        query = (
            ...
            MATCH (c:Community)
            WHERE """
            + neug_in_filter('c.group_id', group_ids, params)
            + '\n'
            + cursor_clause
            + """
            RETURN
